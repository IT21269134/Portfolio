...
const Navbar = ({isDarkMode, setDarkMode}) => {
  ...
  return (
    <>
      <div className="fixed top-0 right-0 w-12/12 -z-10 translate-y-[-80%] opacity-10 dark:hidden">
      ...
      <nav className={`w-full fixed px-5 lg:px-8 xl:px-[8] py-2 flex items-center justify-between z-50 ${isScroll?'bg-white bg-opacity-50 backdrop-blur-lg shadow-sm dark:bg-darkTheme dark:shadow-white/20' :""}`}>
        <a href="#top">
          <Image
            ...
            alt="logo"
            className="w-28 cursor-pointer mr-14 rounded-r-full"
          />
        </a>

        <ul className={`hidden md:flex item gap-6 lg:gap-8 rounded-full px-12 py-3 ${isScroll ? "":" bg-white shadow-sm bg-opacity-50 dark:border dark:border-white/50 dark:bg-transparent"}`}>
          <li>
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={()=>setDarkMode(prev =>!prev)}>
            <Image src={isDarkMode?assets.sun_icon: assets.moon_icon} alt="moon" className="w-5" />
          </button>
          ...
          >
            Contact
            <Image src={isDarkMode?assets.arrow_icon_dark:assets.arrow_icon} className="w-4" alt="contactIcon" />
          </a>

          <button className="bloack md:hidden ml-3" onClick={openMenu}>
